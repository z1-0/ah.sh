...
- **极简入口**：`ah lang <languages...>` 直接进入 `nix develop`。
...
```bash
ah lang nodejs python
ah lang rust go nodejs
```
...
```bash
ah -p dev-templates lang rust go
ah -p devenv       lang rust go
```
...
| 方式                | 你需要做的事             | ah.sh 的优势                               |
| ------------------- | ------------------------ | ------------------------------------------ |
| 手写/维护 flake     | 写配置、改配置、复制粘贴 | `ah lang ...` 用语言列表表达意图，速度更快 |
...
- **小步组合**：先 `ah lang rust` 再加 `go/nodejs`，更容易定位不支持的语言。
...
Commands:
  lang      Create a development session
